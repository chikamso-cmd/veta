...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 z-50 w-full bg-white backdrop-blur-md transition-colors duration-300">
      <nav className="mx-auto flex max-w-8xl items-center justify-between px-6 py-5 lg:px-20">
        ...
        <div className="hidden items-center gap-8 lg:flex">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              href={link.href}
              className="text-sm font-bold text-black transition-colors hover:text-purple-300 "
            >
              {link.name}
            </Link>
          ))}
        ...
        <button
          ...
          className="rounded-lg p-2 text-black lg:hidden"
      ...
      {/* Mobile Navigation */}
      {isOpen && (
        <div className="mx-4 rounded-2xl border border-white/10 bg-black/80 p-5 backdrop-blur-xl lg:hidden">
